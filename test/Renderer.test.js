...
function renderTestName(tagName) {
	return `should render '${tagName}'`;
}

unrenderedTags.concat(intentionallyUnrenderedTags).forEach(tag => {
...
describe('audio', () => {
	...
	test(`${renderTestName('audio')} with src tag and source children`, () => {
		...
		const audio = getHimilayaObject(
			'audio',
			undefined,
			[getHimilayaAttribute('src', 'wubble.mp3')],
			[source1, source2]
		);
		const markdown = renderComponent.audio(audio);
